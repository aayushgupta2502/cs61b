...
class Board {
    ...
    void clear() {
        _numJumps = 0;
        _totalOpen = (EXTENDED_SIDE - 4) * (EXTENDED_SIDE - 4) - 4;
        _numPieces[0] = _totalOpen;
        _numPieces[1] = (EXTENDED_SIDE - 4) * (EXTENDED_SIDE - 4) - _totalOpen;
        _numPieces[2] = 2;
        _numPieces[3] = 2;
        _winner = null;
        _allMoves = new ArrayList<>();

        for (int i = EXTENDED_SIDE * 2 + 3; i < EXTENDED_SIDE * 9 - 3; i++) {
            if (i != EXTENDED_SIDE * 3 - 3 && i != EXTENDED_SIDE * 8 + 2
                    && get(i) != BLOCKED) {
                set(i, EMPTY);
            }
        }
    ...
    PieceColor getWinner() {
        if (_totalOpen == 0) {
            ...
            _winner = EMPTY;
        } else if (_numJumps == EXTENDED_SIDE * 2 + 3) {
            _winner = EMPTY;
        } else {
            _winner = null;
        }
    ...
    boolean legalMove(Move move) {
        ...
        if (get(move.fromIndex()) == BLOCKED
                || get(move.toIndex()) == BLOCKED) {
            return false;
        }
        PieceColor a = get(move.fromIndex());
        if (a != _whoseMove) {
            return false;
        }
        if (move.isPass()) {
        ...
        for (i = 'a'; i <= 'g'; i++) {
            for (j = '1'; j <= '7'; j++) {
                if (get(i, j) == who) {
                    for (int k = -2; k < 3; k++) {
                        for (int l = -2; l < 3; l++) {
                            Move possible = Move.move(i, j, (char) (i + k),
                                    (char) (j + l));
    ...
    void makeMove(Move move) {
        ...
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                neighborIndex = neighbor(move.toIndex(), i, j);
                if (get(neighborIndex) == opponent
                        && neighborIndex != move.fromIndex()) {
                    addUndo(neighborIndex);
    ...
    /** Undo the last move. */
    void undo() {
        while (_undoSquares.get(_undoSquares.size() - 1) != null) {
            PieceColor color = get(_undoSquares.get(_undoSquares.size() - 1));
            if (color != _undoPieces.lastElement()) {
                incrPieces(_undoPieces.lastElement(), 1);
                incrPieces(color, -1);
            }
    ...
    boolean legalBlock(char c, char r) {
        if (get(c, r) == BLOCKED || get(c, r) != EMPTY) {
            return false;
        }
        ...
        char[] rows = new char[] {'1', '2', '3', '4', '5', '6', '7'};

        char reflectedCol = c, reflectedRow = r;

        for (int i = 0; i < 7; i++) {
            if (columns[i] == c) {
                reflectedCol = columns[6 - i];
                break;
            }
        }
        for (int j = 0; j < 7; j++) {
            if (rows[j] == r) {
                reflectedRow = rows[6 - j];
                break;
            }
        }

        if (get(reflectedCol, r) != EMPTY
                || get(c, reflectedRow) != EMPTY
                || get(reflectedCol, reflectedRow) != EMPTY) {
            return false;
        }
        return true;
    ...
    void setBlock(char c, char r) {
        ...
        set(c, r, BLOCKED);
        set(reflectedCol, r, BLOCKED);
        set(c, reflectedRow, BLOCKED);
        set(reflectedCol, reflectedRow, BLOCKED);

        if (reflectedCol == c && reflectedRow == r) {
            _totalOpen -= 1;
        }

        if ((reflectedCol == c && reflectedRow != r)
                || (reflectedCol != c && reflectedRow == r)) {
            _totalOpen -= 2;
        }
