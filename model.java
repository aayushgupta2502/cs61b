...
class Model {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int bonus = (_width * nrows + _height * ncols) * _streakLength;
        if (nrows > 0 & ncols > 0) {
            int add1 = _width * nrows + _height * ncols;
            int sub1 = Math.max(nrows, ncols);
            return bonus + add1 - sub1;
        }
        return bonus + (_width * nrows + _height * ncols);
    }
